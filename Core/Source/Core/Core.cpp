...
VKAPI_ATTR VkBool32 VKAPI_CALL VulkanEngine::debugCallback(
	...
	case VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT:
		severity = "WARNING";
		SetConsoleTextAttribute(hConsole, 14); // stderr yellow foreground
		break;
	case VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT:
		severity = "ERROR";
		SetConsoleTextAttribute(hConsole, 4); // stderr red foreground
		break;
	default:
		severity = "unknown";
	}
	...
	SetConsoleTextAttribute(hConsole, 7); // stderr default
...
void VulkanEngine::createGraphicsPipeline() {
	std::vector<char> vertShaderCode = readFile(VERTEX_SHADERS);
	std::vector<char> fragShaderCode = readFile(FRAGMENT_SHADERS);
...
void VulkanEngine::createTextureImage() {
	...
	createImage(texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage, textureImageMemory, mipLevels, VK_SAMPLE_COUNT_1_BIT);
...
void VulkanEngine::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory, uint32_t mipLevels, VkSampleCountFlagBits numSamples) {
	QueueFamilyIndices indices = findQueueFamilies(physicalDevice);
	uint32_t queueFamilyIndices[] = { indices.graphicsFamily.value(), indices.transferFamily.value() };
...
void VulkanEngine::endSingleTimeCommands(VkCommandBuffer commandBuffer, VkQueue queue, VkCommandPool commandPool) {
...
void VulkanEngine::generateMipmaps(VkImage image, int32_t texWidth, int32_t texHeight, uint32_t mipLevels, VkFormat imageFormat) {
